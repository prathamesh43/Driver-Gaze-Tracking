...
model = load_model('models/0.9934473037719727.h5')
model.summary()
scaler = load(open('scaler_p11.pkl', 'rb'))
print(scaler)
...
imgs = []
#svid = cv2.VideoWriter("vids/" + fname + "_proc2.avi", cv2.VideoWriter_fourcc(*'MJPG'), 25, sze)
...
while True:
    ...
    if ret:
        ...
        if state:
            cv2.putText(img2, "All Good !!", tuple((200, 65)), cv2.FONT_HERSHEY_PLAIN, 2.2, (20, 255, 2), 2)

        else:
            cv2.putText(img2, "EOR !!!!!!!!!", tuple((200, 65)), cv2.FONT_HERSHEY_PLAIN, 2.7, (1, 2, 255), 2)
        #
        # stt2 = time.time()
